...
def allConnected(circuits, boxesCount):
    theIndex = 0
    circuitsCount = len(circuits)
    while theIndex < circuitsCount and len(circuits[theIndex]) == 0: 
        theIndex = theIndex + 1
    if theIndex >= circuitsCount or len(circuits[theIndex]) != boxesCount:
        return False
    theIndex = theIndex + 1
    while theIndex < circuitsCount and len(circuits[theIndex]) == 0: 
        theIndex = theIndex + 1
    return theIndex == circuitsCount
...
with open(filePath, "r") as file:
    ...
    while keepGoing:
        ...
        if allConnected(circuits, boxesCount):
            keepGoing = False
            result = junctionBoxes[boxIndex1][boxXIndex] * junctionBoxes[boxIndex2][boxXIndex]
